...
충북대학교 학연산공동기술연구원의 로비·복도를 바탕으로 만든 **2층 건물 월드**다. 1층에는 가구·출입문·전시물·택배 상자를 유지하고, 2층은 가구와 전시물을 비운 공간에 가벽을 설치했다. 로비 양쪽 계단은 층 사이 중간참에서 180° 돌아 1층·2층을 연결하고 **3층 높이인 6.4m의 도착참까지** 이어진다. 3층 전체 실내 공간은 아직 만들지 않았다.
...
| 층별 실내 높이 | 3.0m |
| 지붕 상단 | z = 6.3m |
| 층간 벽 연결부 | 높이 0.2m, 22개 중 창 위치 2개는 창틀·유리·벽 조각으로 대체 |
| 계단 | 좌우 대칭 2개, 중간참 높이 1.6m·4.8m, 최종 도착 높이 6.4m |
| 계단실 창문 | 뒤벽에 각각 폭 1m × 높이 5m의 세로 창 |
| 패널 조명 | 1층 16개 + 2층 21개 = 37개, 중간참 조명 없음 |
| 물리 환경 | PhysicsScene 1개, DomeLight 1개 |
| 동적 물체 | 1층 택배 상자 16개 |

계단 개구부를 제외한 곳에서 1층 천장과 2층 바닥은 z=3.1m에서 맞닿는다. 계단실은 두 층에 걸쳐 열려 있으며 양층 외벽과 지붕도 계단실 깊이에 맞춰 확장했다. 위로 이어지는 계단 구간의 2층 천장에도 개구부를 냈다.
...
| 동선 | 1층 → 중간참 → 2층 → 중간참 → 3층 높이 도착참 |
| 중간참 높이 / 깊이 | 1.60m·4.80m / 각각 1.20m |
| 전체 단차 수 | 36개, 경사 구간별 9개 |
| 단 높이 | 약 0.1778m |
| 디딤판 깊이 / 두께 | 0.28m / 0.12m |
| 경사 구간 폭 | 1.10m |
| 부속 구조 | 경사 지지대, 양쪽 난간, 6.4m 도착참의 가장자리 가드 |
| 아래 공간 | 채움 형상을 없앤 개방 구조 |

입구에서 보면 왼쪽 계단은 왼쪽으로 올라가 오른쪽으로 2층에 도착하고, 오른쪽 계단은 그 반대다. 2층 도착부 옆을 막던 펜스를 없애고 그 자리에서 같은 방향으로 다시 올라가는 계단을 추가했다. 1층 천장·2층 바닥·2층 천장에는 실제 메시 개구부가 있으며, 디딤판·중간참·난간·지지대에는 정적 충돌을 설정했다. 6.4m 도착참은 폭 2.5m·깊이 1.2m다.

각 계단실 뒤벽에는 z=0.6–5.6m에 걸친 세로 창을 하나씩 배치했다. 기존 불투명 벽과 창을 가로막던 층간 연결부의 표시·충돌을 끄고, 창 옆 벽·상하 벽·금속 창틀·투명 유리를 배치했다. 유리에는 충돌을 유지한다.
...
1층에는 중앙 대형 조명을 포함한 패널 16개가 있다. 2층은 중앙 대형 조명을 제거하고 복도 조명 4개를 보강해 패널 21개를 사용한다. 양쪽 중간참의 벽면 조명은 없애고, 각 2층 계단 입구 위 천장에 패널 조명 1개씩 배치했다. 등 본체 중심 높이는 z=6.16m이며 3층 높이 도착참 아래의 기존 천장 구간에 설치해 계단 개구부와 상승 동선을 피한다. 1층 계단 개구부의 기존 천장등은 비활성화 상태다.
...
- 계단 입구 천장등은 intensity 8000으로 아래를 비춘다. 기존 복도 조명 배치와 세기는 유지한다.
...
│   └── SpawnPoints
├── InterFloorBand              # 연결부 22개, 창 위치 2개는 표시·충돌 해제
├── Stairs_Left                 # 6.4m까지 계단·도착참·뒤벽 세로 창
└── Stairs_Right                # 대칭 구조
```
...
| 패널 조명 37개, 계단 입구 천장등 2개, 중간참 조명 제거 | PASS |
| 양쪽 계단 6.4m 도착·중간참·2층 펜스 제거·천장 개구부 | PASS |
| 양쪽 세로 창의 위치·유리 재질·뒤벽 표시 및 충돌 해제 | PASS |
...
- [3층 높이 계단 연장·세로 창·계단실 천장 조명](docs/31.57_cbnu_haksan_stair_extension_windows.md)
