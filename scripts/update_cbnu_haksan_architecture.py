...
def load_display_walls(path: Path) -> list[dict[str, object]]:
    ...
    for index, wall in enumerate(walls):
        ...
        for field in ("height", "depth", "display_width", "display_height", "mount_clearance"):
...
def main() -> None:
    ...
    args.output.write_text(
        render_layout(walls, column_displays, wall_posters, elevator_doors), encoding="utf-8"
    )
    print(
        f"wrote {args.output} "
        f"({len(walls)} display walls, {len(column_displays)} column displays, "
